...
let sow_seeds board player pit_index =
  let pits, store, opponent_pits =
    match player with
    | Player1 -> board.player1_pits, board.player1_store, board.player2_pits
    | Player2 -> board.player2_pits, board.player2_store, board.player1_pits
  in
  let seeds = pits.(pit_index) in
  let new_pits = Array.copy pits in
  new_pits.(pit_index) <- 0;
  let rec distribute_seeds seeds current_index current_pits current_store =
    if seeds = 0
    then current_pits, current_store
    else (
      let next_index = (current_index + 1) mod 14 in
      let next_pits, next_store =
        if next_index < 6
        then (
          let updated_pits = Array.copy current_pits in
          updated_pits.(next_index) <- updated_pits.(next_index) + 1;
          updated_pits, current_store)
        else if next_index = 6
        then current_pits, current_store + 1
        else if next_index < 13
        then (
          let updated_opponent_pits = Array.copy opponent_pits in
          updated_opponent_pits.(next_index - 7)
          <- updated_opponent_pits.(next_index - 7) + 1;
          current_pits, current_store)
        else current_pits, current_store
      in
      distribute_seeds (seeds - 1) next_index next_pits next_store)
  in
  let final_pits, final_store = distribute_seeds seeds pit_index new_pits store in
  match player with
...
let rec get_move player =
  ...
  then selection
    ...
    (*    let () = print_endline "Bad input, must be 0..5" *)
